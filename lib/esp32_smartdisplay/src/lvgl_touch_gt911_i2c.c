...
void gt911_lvgl_touch_noop_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    // GT911 không khả dụng: touch luôn ở trạng thái released
    data->point.x = 0;
    data->point.y = 0;
...
lv_indev_t* lvgl_touch_init()
{
    ...
    const i2c_config_t i2c_config = {
        ...
        .sda_pullup_en = 1, // Kích hoạt pull-up nội bộ ESP32
        .scl_pullup_en = 1, // Kích hoạt pull-up nội bộ ESP32
        .master = {
    ...
    const esp_err_t touch_err = esp_lcd_touch_new_i2c_gt911(io_handle, &touch_config, &touch_handle);
    if (touch_err != ESP_OK)
    {
        // GT911 không phản hồi: log lỗi và tiếp tục boot (touch vô hiệu hóa),
        // tránh abort làm board reboot loop, để màn hình vẫn hiển thị được.
        log_e("esp_lcd_touch_new_i2c_gt911 failed: %s (%d)", esp_err_to_name(touch_err), touch_err);
        indev->type = LV_INDEV_TYPE_POINTER;
        indev->user_data = NULL;
        indev->read_cb = gt911_lvgl_touch_noop_cb;
        return indev;
    }
