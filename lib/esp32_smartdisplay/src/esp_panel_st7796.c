...
const lcd_init_cmd_t st7796_vendor_specific_init_default[] = {
    // BO HET vendor init: chip nay khong phai ST7796 thuan (HP35006).
    // Bit-bang minimal init (SLPOUT+MADCTL+COLMOD+INVON+DISPON) cho mau dung,
    // nen day cung de trong cho giong het.
};
...
{
    log_v("panel:0x%08x, off:%d => %s", panel, off, off ? "DISPON" : "DISPOFF");
    if (panel == NULL)
        return ESP_ERR_INVALID_ARG;

    // LƯU Ý: esp_lcd_panel_disp_on_off(panel, on_off) truyền "on_off"
    // (true = BẬT màn). Hàm này nhận tham số đặt tên "off" nhưng thật ra
    // là "on_off" theo chuẩn ESP-IDF: true -> DISPON, false -> DISPOFF.
    const st7796_panel_t *ph = (st7796_panel_t *)panel;

    esp_err_t res;
    if ((res = esp_lcd_panel_io_tx_param(ph->io, off ? LCD_CMD_DISPON : LCD_CMD_DISPOFF, NULL, 0)) != ESP_OK)
    {
