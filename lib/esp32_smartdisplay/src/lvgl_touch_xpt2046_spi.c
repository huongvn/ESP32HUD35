...
lv_indev_t *lvgl_touch_init()
{
    ...
    // Create SPI bus only if not already initialized (S035R shares the SPI bus with the display)
    // spi_bus_initialize returns ESP_ERR_INVALID_STATE when the bus is already in use.
    ...
    esp_err_t const bus_err = spi_bus_initialize(XPT2046_SPI_HOST, &spi_bus_config, XPT2046_SPI_DMA_CHANNEL);
